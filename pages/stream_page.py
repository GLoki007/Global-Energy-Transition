...
from data_loader import df_global, df_all
...
def update_streamgraph(view_mode, selected_countries):
    if not selected_countries:
        df_filtered = df_global
        title_suffix = "Global"
    else:
        df_c = df_all[df_all['country'].isin(selected_countries)]
        df_filtered = df_c.groupby('year')[ENERGY_SOURCES].apply(lambda x: x.fillna(0).sum()).reset_index()
        title_suffix = ", ".join(selected_countries) if len(selected_countries) <= 2 else f"{len(selected_countries)} Countries"

    years = df_filtered['year']
    if view_mode == 'percent':
        row_totals = df_filtered[ENERGY_SOURCES].sum(axis=1).replace(0, 1)
        plot_data = df_filtered[ENERGY_SOURCES].div(row_totals, axis=0) * 100
        y_label, hover_suffix = f'Share (%) — {title_suffix}', '%'
    else:
        plot_data = df_filtered[ENERGY_SOURCES]
        y_label, hover_suffix = f'TWh — {title_suffix}', ' TWh'
