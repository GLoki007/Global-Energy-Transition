...
mask_countries = df['iso_code'].notna() & ~df['iso_code'].str.startswith('OWID_', na=False)
...
# Baseline index normalization (First valid year per country reference)
df_valid = df.dropna(subset=['gdp_per_capita', 'co2_per_capita'])
first_valid = df_valid.sort_values('year').groupby('country').first().reset_index()
first_valid = first_valid[['country', 'year', 'gdp_per_capita', 'co2_per_capita', 'renewables_share_energy']]
first_valid = first_valid.rename(columns={
    'year': 'baseline_year',
    'gdp_per_capita': 'gdp_base_2000',
    'co2_per_capita': 'co2_base_2000',
    'renewables_share_energy': 'renew_share_2000'
})
df = df.merge(first_valid, on='country', how='left')
...
# Avoid division by zero/extremely small numbers which causes infinity or extreme spikes.
# If baseline CO2 is 0 or less than 1e-6 (like Burundi), set percentage change to NaN.
df['pct_change_co2'] = np.where(
    df['co2_base_2000'] > 1e-6,
    (df['co2_per_capita'] - df['co2_base_2000']) / df['co2_base_2000'],
    np.nan
)
